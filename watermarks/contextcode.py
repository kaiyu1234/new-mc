...
class SentenceBoundarySemanticExtractor(AbstractContextCodeExtractor):
    """
    基于 K-Means 聚类簇的上下文提取器 + 高频词静默 (Stopword Muting)
    """
    boundary_ids: list[int] = field(default_factory=lambda: [13, 2, 29889]) 
    cluster_file_path: str = "semantic_shuffle_opt_clusters.pt"  # 读取刚才新生成的文件
    
    _class_cache = {}

    def __post_init__(self):
        if not os.path.exists(self.cluster_file_path):
            print(f"[Warning] Cluster file {self.cluster_file_path} not found.")
        self.hash_seeds = [15485863]
        self._boundary_tensor = None

    def _get_cluster_map(self):
        if self.cluster_file_path not in self._class_cache:
            if os.path.exists(self.cluster_file_path):
                self._class_cache[self.cluster_file_path] = torch.load(self.cluster_file_path, map_location="cpu")
            else:
                self._class_cache[self.cluster_file_path] = None
        return self._class_cache[self.cluster_file_path]
    ...
    def extract(self, context: LongTensor) -> bytes:
        ...
        if cluster_map is None:
            ...
            hashes = [self._hash_bucket(int(x), self.hash_seeds[0]) for x in torch.unique(raw_ids)]
            final_hash = min(hashes) if hashes else 0
        else:
            max_id = cluster_map.size(0)
            valid_mask = raw_ids < max_id
            valid_ids = raw_ids[valid_mask]
            
            if len(valid_ids) > 0:
                # 获取这段文本的所有聚类 ID
                clusters = cluster_map[valid_ids]
                
                # : 过滤掉所有 -1 的高频词 (如 "the", "is", "a")
                valid_clusters = clusters[clusters != -1]
                
                unique_clusters = torch.unique(valid_clusters).numpy()
                
                if len(unique_clusters) > 0:
                    hashes = [self._hash_bucket(int(c), self.hash_seeds[0]) for c in unique_clusters]
                    final_hash = min(hashes)  # 只取最坚挺的一个极小值
                else:
                    final_hash = 0
            else:
                final_hash = 0

        return final_hash.to_bytes(4, byteorder='little', signed=False)

@dataclass
class SlidingWindowSemanticExtractor(AbstractContextCodeExtractor):
    """
    基于固定滑动窗口大小的 K-Means 聚类簇上下文提取器 + 高频词静默 (Stopword Muting)
    """
    window_size: int = 15  # 设置滑动窗口大小，例如看前 15 个 token
    cluster_file_path: str = "semantic_shuffle_opt_clusters.pt"
    ...
    def extract(self, context: LongTensor) -> bytes:
        ...
        # 1. 核心修改：不再寻找句子边界，直接提取最后 window_size 个 token 作为上下文
        target_ids = context[-self.window_size:]
