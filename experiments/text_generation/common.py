def get_wps(reweight_type, model_str):
    from watermarks import (
        ...
        SemanticRobustContextCodeExtractor, # <--- 新增这一行
        SentenceBoundarySemanticExtractor,
        SlidingWindowSemanticExtractor,
        NGramHashing,
        Dip_Reweight,
        MC_Reweight,
        STA_Reweight,
    ...
    from watermarks.contextcode import get_boundary_ids
    ...
    my_boundary_ids = get_boundary_ids(tokenizer)
    ...
    # watermark_key_list = [
    #     NGramHashing(
    #         SemanticRobustContextCodeExtractor(n=2, bucket_size=64), 
    ...
    #     NGramHashing(
    #         SentenceBoundarySemanticExtractor(
    #             boundary_ids=my_boundary_ids,  # 动态传入刚刚获取的边界 ID
    #             bucket_size=64,
...
# for Splitmark
# @torch.no_grad()
# def get_split_res_id(vocab_size, input_ids, output_ids, wp, device, eps=0, split_num=None):
#     assert eps <= 1
#     assert eps >= 0
#     decoder_input_ids = output_ids.to(device)
#     label_attention_mask = torch.ones_like(decoder_input_ids).to(device)
...
#     # ====== 核心对齐修复 ======
#     # 将 prompt (input_ids) 和 output 拼接起来，恢复生成时的真实 Context 长度和内容
#     input_ids = input_ids.to(device)
#     full_context_ids = torch.cat([input_ids, decoder_input_ids], dim=1)
...
@torch.no_grad()
def get_split_res_id(vocab_size, input_ids, output_ids, wp, device, eps=0, split_num=None, window_size=15, offset=2, gap_penalty=1.5):
    """
    修改为基于 Smith-Waterman 算法的滑动窗口对齐检测
    """
    assert eps <= 1
    assert eps >= 0
    decoder_input_ids = output_ids.to(device)
    ...
    # 将 prompt (input_ids) 和 output 拼接起来，恢复生成时的真实 Context 长度
    input_ids = input_ids.to(device)
    full_context_ids = torch.cat([input_ids, decoder_input_ids], dim=1)
    prompt_len = input_ids.size(1)
    
    # 假设 batch size = 1
    full_context_seq = full_context_ids[0]
    out_seq = decoder_input_ids[0]
    N = len(out_seq)
    
    # 初始化 Smith-Waterman DP 矩阵
    # dp[i][j] 表示：对齐到生成文本的第 i 个 token (out_seq[i-1])，
    # 并且使用以 full_context_seq 第 j 个 token 结尾的窗口作为上下文的最大得分
    import numpy as np
    dp = np.zeros((N + 1, N + prompt_len + 1), dtype=float)
    
    # 记录每个位置实际贡献的最高分
    max_scores = torch.zeros(decoder_input_ids.shape, device=device)

    for i in range(1, N + 1):
        cur_token = out_seq[i - 1].unsqueeze(0)
        
        # 对应在 full_context 中的理论真实位置是 prompt_len + i
        # 允许的滑动窗口搜索范围：[理论位置 - offset, 理论位置 + offset]
        j_center = prompt_len + i
        j_start = max(prompt_len + 1, j_center - offset)  # 至少得有 prompt
        j_end = min(len(full_context_seq), j_center + offset)
        
        current_max_dp = 0.0
        best_match_score = 0.0
        
        for j in range(j_start, j_end + 1):
            # 提取滑动窗口上下文 (对应生成端的 window_size)
            ctx_start = max(0, j - 1 - window_size)
            pre = full_context_seq[ctx_start : j - 1].unsqueeze(0)
            
            # 计算当前 token 在这个特定上下文下的命中得分
            # 返回的是列表中每个 batch 的得分，取 [0]
            out = wp.get_n_res(pre, vocab_size, cur_token, cur_n=split_num)
            match_score = out[0] 
            
            # Smith-Waterman 状态转移
            # 1. 匹配对齐 (上一个 token 对齐到 j-1 位置)
            align = dp[i - 1, j - 1] + match_score
            # 2. 插入/删除 惩罚
            delete = dp[i - 1, j] - gap_penalty
            insert = dp[i, j - 1] - gap_penalty
            
            # 局部最优，如果低于 0 则断开链路
            dp[i, j] = max(0.0, align, delete, insert)
            
            if dp[i, j] > current_max_dp:
                current_max_dp = dp[i, j]
                best_match_score = match_score

        # 记录这个 token 最终确定的得分 (这里使用实际的 match_score，或者直接将 DP 值用作评估，视你下游的 get_mcmark_acc.py 怎么计算 p-value 而定)
        # 如果你下游的公式是基于独立伯努利试验统计命中次数（即 score=1 or 0），你应该记录 best_match_score
        max_scores[0, i - 1] = best_match_score
        
    return max_scores, label_attention_mask
...
def watermark_score_worker(
    ...
    while not (tqe.is_set() and tq.empty()):
        ...
        # ====== [核心新增：无水印文本的评估替换] ======
        if str(wp_str) == "None":
            # 我们想测试正常文本在 mcmark 水印提取规则下的得分 (False Positive)
            # 直接调用 get_wps 获取 mcmark 的处理器
            target_wps = get_wps(reweight_type="mcmark", model_str=oracle_model_str)
            
            # 强制将当前批次的 wp_str 替换为目标水印规则的字符串表示
            wp_str = repr(target_wps[0]) 
            batch["watermark_processor"] = [wp_str] * len(batch["id"])
        # ============================================
        ...
        input_ids = tbatch["input"]["input_ids"]  # <--- 新增：获取 Prompt 的 IDs

        if "MC_Reweight" in wp_str:  # Mc-mark
            wp = eval(wp_str)
            wp.reset_watermark_key(len(batch["watermark_processor"]))
            
            # 【修复1】深度修改：必须穿透到内部的 Key 开启 ignore_history = False
            for key in wp.watermark_key_list:
                if hasattr(key, "ignore_history"):
                    key.ignore_history = True
            ...
            #     return None
            def extract_n_value(text):
                # 兼容旧版的 "MC_Reweight(n=20)" 和新版的 "MC_Reweight(n=20, entropy_threshold=4.0)"
                pattern = re.search(r"MC_Reweight\(n=(\d+)", text)
                if pattern:
                    return int(pattern.group(1))
                return None
            ...
            # 【修复2 & 3】逐条处理 Batch，并优先使用 JSON 中的真实 output_ids 避免分词器错位
            bsz = len(batch["id"])
            all_raw_scores = []
            all_lens = []
            
            for b_idx in range(bsz):
                # 1. 获取干净的、无 Padding 的 Prompt IDs
                inp_text = batch["input"][b_idx]
                inp_msg = [{"role": "user", "content": inp_text}]
                inp_str = tokenizer.apply_chat_template(inp_msg, add_generation_prompt=True, tokenize=False)
                inp_ids = tokenizer.encode(inp_str, add_special_tokens=False)
                inp_tensor = torch.tensor(inp_ids, device=device).unsqueeze(0)
                
                # ==========================================================
                # 2. 获取无损的 Output IDs（新增了 Dipper 改写检测逻辑）
                # ==========================================================
                out_str = batch["output"][b_idx]
                force_retokenize = False
                original_ids = []
                
                if "output_ids" in batch and batch["output_ids"][b_idx] is not None:
                    # 提取 JSON 里保存的 IDs
                    original_ids = batch["output_ids"][b_idx]
                    if tokenizer.pad_token_id is not None:
                        original_ids = [x for x in original_ids if x != tokenizer.pad_token_id]
                    
                    # 核心校验：把保存的 IDs 解码回文本，看看和当前的文本一不一样
                    # 如果差异很大，说明这是被 Dipper 改写过的文本！
                    decoded_original = tokenizer.decode(original_ids, skip_special_tokens=True).strip()
                    if decoded_original != out_str.strip():
                        force_retokenize = True  # 发现篡改，强制重新分词！
                else:
                    force_retokenize = True

                if force_retokenize:
                    # 对于 Dipper 改写后的文本，必须重新编码
                    out_ids = tokenizer.encode(out_str, add_special_tokens=False)
                else:
                    # 对于未改写的 Baseline 文本，安全使用原始 IDs
                    out_ids = original_ids
                
                out_tensor = torch.tensor(out_ids, device=device).unsqueeze(0)
                # ==========================================================
                
                # 3. 调用你的 get_split_res_id 计算得分
                scores, label_attention_mask = get_split_res_id(
                    vocab_size, inp_tensor, out_tensor, wp, device, eps=eps, split_num=cur_n
                )

                # 4. 忽略前2个 token，计算最终分数
                label_attention_mask[0, :2] = 0
                scores = scores * label_attention_mask
                
                all_raw_scores.append(scores.sum().item())
                all_lens.append(label_attention_mask.sum().item())
        ...
        #     assert cur_n is not None
        #     scores, label_attention_mask = get_split_res_id(
        #         vocab_size,input_ids, output_ids, wp, device, eps=eps, split_num=cur_n
        #     )
