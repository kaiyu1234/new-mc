def get_wps(reweight_type, model_str):
    from watermarks import (
        ...
        FastRecoverySemanticExtractor,  # <--- 导入这个新类
        AnchorSemanticContextCodeExtractor,
        RobustMinHashContextCodeExtractor,
        HybridSemanticContextCodeExtractor,
        NGramHashing,
        Dip_Reweight,
        MC_Reweight,
        STA_Reweight,
    ...
    # watermark_key_list = [
    #     NGramHashing(
    #         HybridSemanticContextCodeExtractor(window_size=20, n_local=1, bucket_size=64), 
    #         ignore_history=False
    #     )
    # ]
    # watermark_key_list = [
    #     NGramHashing(
    #         RobustMinHashContextCodeExtractor(window_size=30, num_hashes=1, bucket_size=64), 
    #         ignore_history=True  # <--- [核心修改]: 强制为 True，允许同一个 MinHash 连续注入水印！
    #     )
    # ]

    # watermark_key_list = [
    #     NGramHashing(
    #         AnchorSemanticContextCodeExtractor(anchor_freq=4, bucket_size=64), 
    #         ignore_history=True  # 必须为 True！因为锚点会使得几个连续词的 Context 相同，防止防重复机制误杀
    #     )
    # ]
