...
class DipperParaphraser(object):
    def __init__(self, model="kalpeshk2011/dipper-paraphraser-xxl", verbose=True):
        ...
        # [修改] 添加 torch_dtype=torch.float16 以节省一半显存
    ...
    def paraphrase(
        ...
        **kwargs,
        ...
        for sent_idx in range(0, len(sentences), sent_interval):
            ...
            # [FIX] 开启截断，防止超过 T5 的 512 长度限制导致报错
    ...
    try:
        output = client.chat.completions.create(
            ...
            messages=[
                ...
                # [新增] 系统指令，不要废话
                {"role": "system", "content": "你是文本改写助手，仅进行少量的同义词替换，直接输出改写后的文本，不要任何额外的开头、结尾和解释，严格保留原句结构和长度."},
                {"role": "user", "content": prompt}
            ],
            seed=seed,
            top_p=0.1,  # 仅采样概率最高的10%token，进一步降低随机性
            frequency_penalty=0.1,  # 轻微惩罚重复token，避免无意义改写
            presence_penalty=0.0,  # 不惩罚新token，保证语义完整
...
#             prompt = f"""请严格按照以下规则改写文本，直接输出结果即可：
# 1. 仅替换少量非核心、非专业术语的同义词；
# 2. 原文本的句子结构不变，语序、段落划分不变，不做句式调整；
# 3. 改写后文本长度与原文基本一致，不增删额外字符；
# 4. 完全保留原文本的所有语义和信息，语义信息和原文一致.

            
            prompt = f"""请严格按照以下规则改写文本，直接输出结果即可：
1. 仅替换少量非核心、非专业术语的同义词；
2. 原文本的句子结构、语序、段落划分不变，不做句式调整；
3. 改写后文本长度与原文基本一致，不增删任何字符；
4. 完全保留原文本的所有语义和信息，无语义偏差.
...
# 2. Only replace a small number of synonyms,Preserve the original meaning and sentence structure as much as possible.
