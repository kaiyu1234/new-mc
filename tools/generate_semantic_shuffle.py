...
def generate_semantic_shuffle(freq_file,model_name, split_num, save_path, high_freq_ratio=0.1, softness=0.2):
    ...
    if freq_file and os.path.exists(freq_file):
        ...
        if len(counts) != vocab_size:
            print(f"Warning: Count file size ({len(counts)}) != Model Vocab ({vocab_size})")
            # 如果 counts 少了，补0；如果多了，截断
            if len(counts) < vocab_size:
        ...
        # [关键] 排序逻辑
        # 我们要高频词在前面。argsort 默认是升序。
        # 所以我们可以对负数排序，或者 argsort 后反转
        sorted_indices = np.argsort(-counts) # 降序排列，counts 最大的在最前面
        
        # 打印一下最高频的几个 ID 供检查 (对于 Llama/Mistral, 通常是 <unk>, <s>, ., ,, the 等)
        print(f"Top 10 frequent tokens: {sorted_indices[:10]}")
    else:
        print("[Warning] No frequency file provided! Falling back to Embedding Norm (Not Robust!).")
        norms = np.linalg.norm(embeddings, axis=1)
        # Norm 越小通常被认为是高频（靠近原点），所以升序排列
        sorted_indices = np.argsort(norms)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freq_file", type=str, default=None, help="Path to pre-computed token frequency .pt file")
