...
def generate_semantic_shuffle(freq_file, model_name, split_num, save_path, high_freq_ratio=0.1, softness=0.2):
    ...
    print("-" * 30)
    print(f"Max first Cluster Size: {max(sizes)}")
    print(f"Min first Cluster Size: {min(sizes)}")
    print("-" * 30)
    ...
    print("Generating pure Semantic Cluster Map for Context Extraction...")
    
    # 创建一个与词表等大的 Tensor，默认全部填入 -1（代表高频词/无语义词，将在提取时被忽略）
    cluster_map = torch.full((vocab_size,), -1, dtype=torch.long)
    
    # 将具有明确语义的词，映射到它们所属的 K-Means 簇 ID 上
    # clusters 的 key 是最终的 cluster_id，value 是对应的 token idx 列表
    # for cluster_id, token_indices in clusters.items():
    ...
    # 将聚类映射表保存为独立文件
    cluster_save_path = save_path.replace(".pt", "_clusters.pt")
    print(f"Saving Cluster Map to {cluster_save_path}...")
    torch.save(cluster_map, cluster_save_path)
